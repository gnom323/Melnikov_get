...
def can_rearrange_cities(cities):
    ...
    for i in range(len(cities)):
        if graph.get(first_letters[i]) == None:graph[first_letters[i]] = list(last_letters[i])
        else: graph[first_letters[i]].append(last_letters[i])
...
cities6 = ["manchester", "riga", "abakan", "nottingham"] #обратить внимание
print(can_rearrange_cities(cities6))