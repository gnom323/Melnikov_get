...
plt.plot(nu_1, reg1.predict(nu_1.reshape(10,1)),'grey',label=f"Аппрокс. прямая k'1 = {1/reg1.coef_[0]:.5f}")

plt.plot(nu_2.reshape(10,1),n,':s', label="T=4,9 H")
plt.plot(nu_2, reg2.predict(nu_2.reshape(10,1)),'grey',label=f"Аппрокс. прямая k'2 = {1/reg2.coef_[0]:.5f}")
plt.plot(nu_3.reshape(10,1),n,':+', label="T=14,8 H", markersize=8)
plt.plot(nu_3, reg3.predict(nu_3.reshape(10,1)),'grey',label=f"Аппрокс. прямая k'3 = {1/reg3.coef_[0]:.5f}")
plt.plot(nu_4.reshape(10,1),n,':^', label="T=19,7 H", markersize=7)
plt.plot(nu_4, reg4.predict(nu_4.reshape(10,1)),'grey',label=f"Аппрокс. прямая k'4 = {1/reg4.coef_[0]:.5f}")
plt.plot(nu_5.reshape(10,1),n,'v', label="T=24,6 H", markersize=7)
plt.plot(nu_5, reg5.predict(nu_5.reshape(10,1)),'grey',label=f"Аппрокс. прямая k'5 = {1/reg5.coef_[0]:.5f}")
